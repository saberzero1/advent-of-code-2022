...
# example: 37-42,33-42
# Find the number of lines where one range overlaps the other
# example: 37-42,33-42 is valid because 37-42 is fully contained in 33-42
# example: 37-42,33-40 is valid because 37-42 overlaps 33-40
# example: 37-42,43-50 is invalid because 37-42 does not overlap 43-50
...
for line in lines:
    ...
    # check if one range partly overlaps the other
    if (range1[0] >= range2[0] and range1[0] <= range2[1]) or (range1[1] >= range2[0] and range1[1] <= range2[1]) or (range1[0] <= range2[0] and range1[1] >= range2[1]):
        # if so, increment the count
        count += 1

# write the count to the file
with open('Day 4/out2.txt', 'w') as f:
    f.write(str(count))
