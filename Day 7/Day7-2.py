...
for child in root.get_all_unique_children():
    # find the directory with smallest size of at least required
    # only if the size is currently more than required
    # only if smaller than the current smallest
    if child.get_size() >= required and child.get_size() < current:
        current = child.get_size()

# write the sum to the file
with open('Day 7/out2.txt', 'w') as f:
    f.write(str(current))
